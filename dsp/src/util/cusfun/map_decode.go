...
func Map2ModelMap(input map[string]interface{}) map[string]interface{} {
	outputMap := make(map[string]interface{}, len(input))
...
func Map2Model(input map[string]interface{}, output interface{}) error {
	outputMap := make(map[string]interface{}, len(input))
	for key, value := range input {
		k := Marshal(key)
		outputMap[k] = value
	}
	return mapstructure.Decode(outputMap, output)
}
